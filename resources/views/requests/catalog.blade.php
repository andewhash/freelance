        ...
        <div class="col-md-3">
            <div class="card shadow-sm mb-4" style="border: none">
                ...
                <div class="card-body">
                    ...
                    <a href="{{ route('requests.catalog') }}" class="btn btn-outline-secondary btn-sm">Сбросить</a>
                </div>
            </div>
        </div>
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb bg-white p-3 shadow-sm rounded">
                    ...
                    @foreach($breadcrumbs as $crumb)
                        @if(!$loop->last)
                        <li class="breadcrumb-item"><a href="{{ route('requests.catalog', ['category' => $crumb['id']]) }}">{{ $crumb['name'] }}</a></li>
                        @else
                ...
                <div class="col-md-4 mb-4">
                    <div class="card h-100 shadow-sm">
                        <div class="card-body">
                            ...
                            <p class="card-text text-muted">{{ $order->country }}</p>
                            <p class="card-text">{{ Str::limit($order->description, 100) }}</p>
                            <div class="d-flex justify-content-between align-items-center">
                                <div class="badge bg-primary">{{ $order->category->name ?? '' }}</div>
                                <small class="text-muted">{{ $order->price }} сум</small>
                            </div>
                        </div>
                        <div class="card-footer bg-white">
                            <a href="{{ route('requests.show', $order->id) }}" class="btn btn-sm btn-primary">Подробнее</a>
                        </div>
                    </div>
                </div>
                @endforeach
